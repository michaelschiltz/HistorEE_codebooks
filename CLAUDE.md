...
Every Markdown file in the repo is **soft-wrapped**: one physical line per paragraph, list item and blockquote, with blank lines between blocks. Do not hard-wrap prose at a column width — let the editor wrap. Hard-wrapped entries have had to be re-flowed by hand after the fact. Headings, tables, `---` rules, and fenced or indented code are exempt. This is a house-style rule, not a data rule, but it applies to every file an assistant writes. **`records/` is exempt from both rules and from tidying of any kind**: it is the tracked batch record — priors, prompts, coding notes, proposed rows — preserved verbatim so that what the deposit claims about how it was made can be checked. Never reformat it. `proposed*/` stays ignored and holds only work in flight. See `records/README.md`. **Tables are exempt from the wrapping rule but have one of their own: they are padded to equal column widths**, so that a table is legible in the raw file and not only after rendering. `scripts/check_tables.py` enforces it repo-wide and `--fix` aligns in place; the generators have always produced aligned tables, and since 2026-09-08 hand-written ones are held to the same standard.
