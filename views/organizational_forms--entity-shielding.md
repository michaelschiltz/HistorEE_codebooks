...
No mechanism filter (this dataset has no mechanism characteristic). **30 forms × 3 characteristics.** This is NOT the full characteristic set: comparative claims run on a declared component set only (`CHARACTER-CODING.md`). At this *n* the matrix is a coverage map, not evidence.

## Matrix

| form | `AP1` | `AP2` | `AP4` |
|---|---|---|---|
| `asiento_averia` | .NR | P | .NA |
| `avariz_vakfi` | .NR | .NR | 1 |
| `bazacle_mill` | P | 1 | .NA |
| `begijnhof` | .NR | .NR | 1 |
...
| `ortoq_equity` | -- | -- | -- |
| `ortoq_loan` | -- | -- | -- |
| `partenrederij` | .NR | .NR | .NA |
| `qirad` | .NR | .NR | .NA |
| `shenhui_gu` | -- | -- | -- |
| `societas_maris` | P | .NR | .NA |
| `voc_1602` | 1 | 1 | .NA |
| `voc_1612` | 1 | 1 | .NA |
| `voc_1623` | 1 | 1 | .NA |
| `waqf_khayri` | 1 | 1 | 1 |
...
- **`AP1` entity shielding weak** (ternary) — ternary: `1` present, `P` partial, `0` absent
- **`AP2` liquidation protection** (ternary) — ternary: `1` present, `P` partial, `0` absent
- **`AP4` separation from founder estate** (ternary) — ternary: `1` present, `P` partial, `0` absent
