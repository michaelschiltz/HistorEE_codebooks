...
No mechanism filter (this dataset has no mechanism characteristic). **32 forms × 4 characteristics.** This is NOT the full characteristic set: comparative claims run on a declared component set only (`CHARACTER-CODING.md`). At this *n* the matrix is a coverage map, not evidence.
...
- **`TS1` perpetual succession** (ternary) — ternary: `1` present, `P` partial, `0` absent
- **`TS2` venture horizon** (nominal) — `0` = open; `1` = fixed-term; `2` = single-venture
- **`TS3` revocability** (ternary) — ternary: `1` present, `P` partial, `0` absent
- **`TS4` mutability of purpose** (ternary) — ternary: `1` present, `P` partial, `0` absent
